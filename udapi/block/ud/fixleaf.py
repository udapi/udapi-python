...
class FixLeaf(Block):
    ...
    def __init__(self, deprels='aux,cop', **kwargs):
        """
        Args:
        deprels: comma-separated list of deprels to be fixed. Default = aux,cop.
        """
    ...
    def process_node(self, node):
        for deprel in self.deprels:
            if node.udeprel == deprel:
                ...
                children = [c for c in children if not re.match(r'^(fixed|conj|cc|punct|goeswith|reparandum)$', c.udeprel)]
