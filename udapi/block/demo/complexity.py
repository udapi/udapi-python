...
def is_relcl(node):
    ...
    if node.deprel == 'acl:relcl':
        return True
    return node.udeprel == 'acl' and next((c for c in node.children if 'Rel' in c.feats['PronType']), None)
...
def is_complex_nominal(node):
    ...
    """
    return is_np(node) and len([n for n in node.children if n.deprel not in ('conj', 'punct', 'case', 'cc', 'dep', 'cop')]) > 1
...
class Complexity(BaseWriter):
    ...
    def process_tree(self, root):
        ...
        while queue:
            ...
            if self.is_finite_clause_head(node):
        ...
        self.report("adjectivized_predicates", [[n] for n in allnodes if self.is_adjectivized_predicate(n)])
        self.report("controlled_predicates", [[n] for n in allnodes if self.is_controlled_predicate(n)])
        ...
        # TODO postponed_adj_mod si přidal Martin
    ...
    def is_finite_clause_head(self, node):
        ...
        * TODO: by uncommenting the nsubj/csubj line, we find few more real clause heads, but also some false positives.    
        """
        # TODO appos
        if ((node.udeprel in {'root', 'conj', 'acl', 'advcl', 'ccomp', 'csubj', 'obl', 'parataxis', 'amod', 'dep', 'orphan'} and self.is_finite_verb(node))
    ...
    def is_controlled_predicate(self, node):
    ...
    def get_t_units(self, main_heads):
        ...
        for main_head in main_heads:
            ...
            for dep_clause_head in dep_heads:
                results.append(main_clause + self.expand_subtree([dep_clause_head], 'subtree')) # TODO subtree_within_clause?
        return results
